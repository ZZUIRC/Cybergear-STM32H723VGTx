...
void CybergearDriver::dump_motor_param()
{
    ...
    index_array.push_back(ADDR_RUN_MODE);
    index_array.push_back(ADDR_IQ_REF);
    index_array.push_back(ADDR_SPEED_REF);
    index_array.push_back(ADDR_LIMIT_TORQUE);
    index_array.push_back(ADDR_CURRENT_KP);
    index_array.push_back(ADDR_CURRENT_KI);
    index_array.push_back(ADDR_CURRENT_FILTER_GAIN);
    index_array.push_back(ADDR_LOC_REF);
    index_array.push_back(ADDR_LIMIT_SPEED);
    index_array.push_back(ADDR_LIMIT_CURRENT);
    index_array.push_back(ADDR_MECH_POS);
    index_array.push_back(ADDR_IQF);
    index_array.push_back(ADDR_MECH_VEL);
    index_array.push_back(ADDR_VBUS);
    index_array.push_back(ADDR_ROTATION);
    index_array.push_back(ADDR_LOC_KP);
    index_array.push_back(ADDR_SPD_KP);
    index_array.push_back(ADDR_SPD_KI);

    for (std::vector<uint16_t>::iterator it = index_array.begin(); it != index_array.end(); ++it) {
        uint16_t index = *it;
        read_ram_data(index);
        HAL_Delay(1);
    }
  ...
  uint8_t can_id, uint8_t cmd_id, uint16_t option, uint8_t len, uint8_t * data)
{
   ...
   // printf("Sending CAN message, ID: 0x%08X, Data: ", id);
    for (int i = 0; i < len; i++)
    {
      //  printf("%02X ", data[i]);
    }
 //   printf("\n");
    ...
    {
      //  printf("Failed to send CAN message, ID: 0x%08X\n", id);
        return;
    }
    HAL_Delay(1);
    ++send_count_;
...
//  motor_status_.stamp_usec = micros();
  motor_status_.stamp_usec = HAL_GetTick();
...
//    motor_param_.stamp_usec = micros();
	  motor_param_.stamp_usec = HAL_GetTick();
  }
}
//
